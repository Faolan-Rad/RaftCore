...
public class RaftNodeTest
{
    ...
    public void TestGetCommittedEntries()
    {
        ...
        var entry1 = new LogEntry<string>(1, 1, "2");
        var entry2 = new LogEntry<string>(1, 2, "4");

        node.AppendEntries(1, 2, 0, 0, [entry1, entry2], 0);
    ...
    public void TestAppendEntriesConsistencyCheck()
    {
        ...
        var entry1 = new LogEntry<string>(1, 0, "2");
        var res = node.AppendEntries(1, 1, 0, 0, [entry1], 0);
        Assert.True(res.Value);
        Assert.Equal("2", node.Log[0].Command);

        // Append dummy valid entry, passes the consistency check:
        // Log[0].TermNumber == 1
        var entry2 = new LogEntry<string>(1, 1, "3");
        res = node.AppendEntries(1, 1, 0, 1, [entry2], 0);
        Assert.True(res.Value);
        ...
        // Log[0].TermNumber != 0
        // 1 != 0
        var entry3 = new LogEntry<string>(1, 2, "4");
        res = node.AppendEntries(3, 1, 0, 0, [entry3], 0);
        Assert.False(res.Value);
    }
    ...
    public void TestRequestVoteConsistencyCheck()
    {
        ...
        var entry1 = new LogEntry<string>(1, 0, "2");
        var entry2 = new LogEntry<string>(2, 1, "3");
        node.AppendEntries(1, 1, 0, 0, [entry1, entry2], 0);
        // node.Log.Count - 1 --> 1
        Assert.Equal(1, node.Log.Count - 1);
        // node.LastLogTerm   --> 2
        Assert.Equal(2, node.Log[^1].TermNumber);

        // pass consistency check
        var res = node.RequestVote(2, 2, 1, 2);
        Assert.True(res.Value);

        // outdated last log index
        // if lastLogIndex >= Log.Count - 1 is false, no vote
        res = node.RequestVote(2, 2, 0, 2);
        Assert.False(res.Value);

        // outdated last log term
    ...
    public void TestMakeRequestToFollower()
    {
        ...
        RaftNode<uint, string>[] nodes = TestMethods.ConfigureAndRunRaftCluster(2, SM.Dictionary);
    ...
    public void TestApplyEntry()
    {
        ...
        var entry1 = new LogEntry<string>(1, 1, "2");
        var entry2 = new LogEntry<string>(1, 2, "4");
        var entry3 = new LogEntry<string>(1, 3, "-1");

        node.AppendEntries(1, 2, 0, 0, new List<LogEntry<string>>() { entry1, entry2, entry3 }, 1);
    ...
    {
        var node = TestMethods.CreateNode();
        var entry = new LogEntry<string>(1, 1, "2");
