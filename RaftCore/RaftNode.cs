...
public enum NodeState
{
    ...
    /// <summary>
    /// Represents the node has been stopped (see <see cref="RaftNode{TID,TCommand}.Stop"/> and won't respond to RPCs.
    /// </summary>
    Stopped
};
...
/// </summary>
public class RaftNode<TID, TCommand>
    where TID : unmanaged, IEqualityOperators<TID, TID, bool>
{
    ...
    /// </summary>
    public TID NodeId { get; }

    /// <summary>
    /// State machine this node replicates.
    /// </summary>
    public IRaftStateMachine<TCommand> StateMachine { get; private set; }

    /// <summary>
    /// Represents the cluster this node is part of.
    /// </summary>
    public RaftCluster<TID, TCommand> Cluster { get; private set; }

    /// <summary>
    /// Log of entries to replicate.
    /// </summary>
    public List<LogEntry<TCommand>> Log { get; private set; }
    ...
    /// </summary>
    public TID? LeaderId { get; private set; } = null;

    /// <summary>
    /// ID of the candidate that received vote in current term, if any.
    /// </summary>
    public TID? VotedFor { get; private set; } = null;
    ...
    /// </summary>
    public ConcurrentDictionary<TID, int> NextIndex { get; }

    /// <summary>
    /// For each node, index of highest log entry known to be replicated on that node.
    /// </summary>
    public ConcurrentDictionary<TID, int> MatchIndex { get; }
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RaftNode{TID,TCommand}"/> class.
    /// </summary>
    /// <param name="nodeId">The node's ID</param>
    /// <param name="stateMachine"><see cref="IRaftStateMachine{TCommand}"/> to replicate</param>
    public RaftNode(TID nodeId, IRaftStateMachine<TCommand> stateMachine)
    {
        this.NodeId = nodeId;
        this.StateMachine = stateMachine;
        this.Log = [];

        _electionTimer = new Timer(TriggerElection);
        _heartbeatTimer = new Timer(SendHeartbeats);
    ...
    /// </summary>
    /// <param name="cluster"><see cref="RaftCluster{TID,TCommand}"/> instance containing all the nodes in the cluster</param>
    public void Configure(RaftCluster<TID, TCommand> cluster)
    {
    ...
    /// Initializes the timers and state appropriate to the node's state.
    /// </summary>
    public void Run()
    {
        if (Cluster == null)
    ...
    public Result<bool> AppendEntries(int term, TID leaderId, int prevLogIndex, int prevLogTerm,
                              List<LogEntry<TCommand>> entries, int leaderCommit)
    {
        ...
        if (leaderCommit > CommitIndex)
        {
            ...
            CommitIndex = Math.Min(leaderCommit, Log[^1].Index);
    ...
    /// </summary>
    /// <param name="command"><typeparamref name="TCommand"/> forming a command recognizable by the state machine</param>
    public void MakeRequest(TCommand command)
    {
        if (NodeState == NodeState.Leader)
        {
            ...
            var entry = new LogEntry<TCommand>(CurrentTerm, Log.Count, command);
            Log.Add(entry);
        }
    ...
    public List<LogEntry<TCommand>> GetCommittedEntries()
    {
    ...
    private void ResetElectionTimer()
    {
        if (NodeState != NodeState.Leader)
        {
            _electionTimer.Change(ElectionTimeoutMS, ElectionTimeoutMS);
        }
    }
    ...
    private void ResetHeartbeatTimer()
    {
        if (NodeState == NodeState.Leader)
        {
            _heartbeatTimer.Change(0, ElectionTimeoutMS / 2);
        }
    }
    ...
    {
        return (Log.Count > 0) ? Log[^1].TermNumber : 0;
    }
    ...
    private void SendHeartbeats(object arg)
    {
        ...
        Parallel.ForEach(nodes, nodeId =>
        {
            if (!NextIndex.TryGetValue(nodeId, out int value)) return; // Prevents errors when testing
            var prevLogIndex = Math.Max(0, value - 1);
            int prevLogTerm = (Log.Count > 0) ? prevLogTerm = Log[prevLogIndex].TermNumber : 0;

            List<LogEntry<TCommand>> entries;

            if (Log.Count > value)
            {
                LogMessage("Log Count: " + Log.Count + " -- Target node[nextIndex]: " + nodeId + " [" + value + "]");
                entries = Log.Skip(value).ToList();
            }
            else
            {
            ...
            {
                LogMessage("Failed AE to " + nodeId + ". Setting nextIndex to " + value);
