...
/// </summary>
public class APIRaftConnector : IRaftConnector<uint, string>
{
    ...
    /// </summary>
    /// <param name="command"><see cref="string"/> containing the request to send to the node</param>
    ...
    public Result<bool> AppendEntries(int term, uint leaderId, int prevLogIndex, int prevLogTerm, List<LogEntry<string>> entries, int leaderCommit)
    {
        // convert params to json
    ...
    private async void SendMakeRequest(string command)
    {
        ...
        var response = await _client.PostAsync(BaseURL + "makerequest", content);
    }
    ...
    private async Task<string> SendRequestVote(int term, uint candidateId, int lastLogIndex, int lastLogTerm)
    {
        ...
        var response = await _client.PostAsync(BaseURL + "requestvote", content);
    ...
    private async Task<string> SendAppendEntries(int term, uint leaderId, int prevLogIndex, int prevLogTerm, List<LogEntry<string>> entries, int leaderCommit)
    {
        ...
        var response = await _client.PostAsync(BaseURL + "appendentries", content);
    ...
    {
        var response = await _client.GetAsync(BaseURL + "test");
    }
